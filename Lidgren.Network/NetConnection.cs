...
namespace Lidgren.Network
{
	...
	public partial class NetConnection
	{
        ...
        /*
         *  Description:    The event will trigger when acknowledge was received.
         *  Author:         Added by CClien
         *  Date:           2012/11/30
         */
        internal event EventHandler<AckRecvEventArgs> onAckRecv;
		...
		internal void Heartbeat(float now, uint frameCounter)
		{
			...
			if ((frameCounter % m_messageCoalesceFrames) == 0) // coalesce a few frames
			{
				...
				while (m_queuedIncomingAcks.TryDequeue(out incAck))
				{
                    ...
                    /*
                         *  Description:    Trigger the onAckRecv Event.
                         *  Author:         Added by CClien
                         *  Date:           2012/11/30
                         */
                    if (onAckRecv != null)
                        onAckRecv.Invoke(this, new AckRecvEventArgs(incAck.Item2));
				}
			}
    ...
    /*
     *  Description:    The Event Arguments of onAckRecv event.
     *  Author:         Added by CClien
     *  Date:           2012/11/30
     */
    public class AckRecvEventArgs : EventArgs
    {
        public int SeqNum;

        internal AckRecvEventArgs(int seq)
        {
            SeqNum = seq;
        }
    }
